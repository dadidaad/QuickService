...
namespace QuickServiceWebAPI.Repositories.Implements
{
    public class GroupRepository : IGroupRepository
    {
        ...
        public async Task<Group> GetGroupById(string groupId)
        {
            try
            {
                Group group = await _context.Groups.Include(g => g.Users).Include(b => b.BusinessHour).Include(u => u.GroupLeaderNavigation.Role).FirstOrDefaultAsync(x => x.GroupId == groupId);
                return group;
            }
            catch (Exception ex)
            {
